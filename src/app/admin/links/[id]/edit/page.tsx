...
import { handleUpdateLink } from "@/actions/links";
...
export default function EditLinkPage() {
  ...
  useEffect(() => {
    const fetchLink = async () => {
      try {
        const link: LinkType = await getLinkById(id);
        setFormData({
          name: link.name,
          url: link.url,
        ...
        error("加载失败", err instanceof Error ? err.message : "加载友链失败");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await handleUpdateLink(id, form);
      ...
      dismiss(toastId);
      error("更新友链失败", err instanceof Error ? err.message : "更新友链失败");
    } finally {
